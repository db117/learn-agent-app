...
const modelServer = createServer(async (request, response) => {
    ...
    response.end(JSON.stringify({
      ...
      choices: [{
        ...
        message: firstRequest
          ? { role: "assistant", content: null, tool_calls: [{ id: "native-check-call", type: "function", function: { name: "echo", arguments: JSON.stringify({ text: "native-check" }) } }] }
          : { role: "assistant", content: hasToolResult ? "Echo observed: native-check" : "native-check response" },
  ...
  const chunks = firstRequest
    ? [
        { delta: { role: "assistant", reasoning_content: "native-check reasoning", tool_calls: [{ index: 0, id: "native-check-call", type: "function", function: { name: "echo", arguments: JSON.stringify({ text: "native-check" }) } }] }, finish_reason: null },
  ...
  for (const chunk of chunks) {
    response.write(`data: ${JSON.stringify({ id: `native-check-${modelRequests.length}`, object: "chat.completion.chunk", model: "native-check", choices: [{ index: 0, flag: 0, delta: chunk.delta, finish_reason: chunk.finish_reason }] })}\n\n`);
...
try {
  ...
  console.log(JSON.stringify({ native: "UP", health, selfTest: selfTest.id, sqlite: "UP", adk: "UP", tool: "UP", sse: "UP", formalPost: receipt.runId, messages: detail.messages.length }));
} catch (error) {
