...
- LearningJourney：某个 Learner 的有序学习路径；一个 Learner 可以拥有多个历史 Journey，同一语言最多一个 `ACTIVE`
  Journey；Journey 拥有自己的内容快照和学习进度。
...
LearningJourney 在全部 LearningPathItem 完成或跳过时可以完成；显式恢复跳过项时可以重新进入 `ACTIVE`。
