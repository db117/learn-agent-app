...
`sequence/code` 作为稳定的并列排序键。已通过的技能成为 `COMPLETED`，跳过的技能
成为 `SKIPPED`，剩余队列只由第一个 `PENDING` 项成为 `CURRENT`。

ProgressService 负责状态联动：

- 诊断通过的技能直接标记 `PASSED`，未通过的技能进入 `READY`。
- 技能评估通过后完成当前 Path item 并移动到下一个 pending 技能。
- 技能评估失败保留 `LEARNING`；Skip 只写 `SKIPPED` 历史，不写 100 分。
